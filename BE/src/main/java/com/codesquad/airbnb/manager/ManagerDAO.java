...
public class ManagerDAO {
    ...
    public Boolean canReserve(Long roomId, ReservationDate reservationDate) {
        ...
        String sql =
                "SELECT count(*) AS count FROM rooms r INNER JOIN dates d on r.room_id = d.room_id WHERE ((? BETWEEN d.check_in_date AND d.check_out_date ) OR (? BETWEEN d.check_in_date AND d.check_out_date ) OR (? < d.check_in_date AND ? > d.check_out_date)) AND r.room_id = ? GROUP BY r.room_id";
        ...
        return this.jdbcTemplate.query(sql,
                new Object[] {checkInDate, checkOutDate, checkInDate, checkOutDate, roomId},
                resultSetExtractor);
    }
    ...
    public Confirmation showBillAndReview(Long roomId, ReservationDate reservationDate) {
        ...
        RowMapper<Confirmation> reservationRowMapper = new RowMapper<Confirmation>() {
            ...
            public Confirmation mapRow(ResultSet rs, int rowNum) throws SQLException {
                ...
                int length = (int) (ChronoUnit.DAYS.between(reservationDate.getCheckInDate(),
